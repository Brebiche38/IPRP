...
void* si_routine(void* arg) {
	...
	if ((subscribe_socket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1) {
...
iprp_sender_ifaces_t *find_si_in_array(iprp_sender_ifaces_t *si, iprp_sender_ifaces_t *array, int count) {
	for (int i = 0; i < count; ++i) {
		if (si->sender_addr.s_addr == array[i].sender_addr.s_addr
				&& si->group_addr.s_addr == array[i].group_addr.s_addr) {
...
bool find_si_in_list(iprp_sender_ifaces_t *si) {
	...
	while(iterator != NULL) {
		...
		if (sender->sender_addr.s_addr == si->sender_addr.s_addr
				&& sender->group_addr.s_addr == si->group_addr.s_addr) {
			// We have found the entry
			return true;
		}
...
void membership(struct in_addr group_addr, struct in_addr src_addr, struct in_addr host_addr, bool add) {
	...
	ssm_request.imr_address.s_addr = htonl(INADDR_ANY);
	ssm_request.imr_ifindex = 0;
	...
	if (add) {
		printf("Added membership to %x from %x\n", group_addr.s_addr, src_addr.s_addr);
	} else {
